...
// =============================
const ALL_THEMES = [
  'cyber', // toujours débloqué côté UI
  'neon',
  'nuit','nature','bubble','retro',
  'vitraux','angelique','luxury','space','arabic','grece','japon'
];
...
function getCurrentTheme() {
  return localStorage.getItem('themeVBlocks') || 'cyber';
}
...
function applyLocalTheme(themeKey) {
  const t = normalizeThemeKey(themeKey || 'cyber');
  ...
  if (link) link.href = themesWithOwnCss.has(t) ? `themes/${t}.css` : 'themes/themes.css';
...
async function ensureUserRow() {
  try {
    ...
    if (rpc && rpc.error) {
      ...
      var finalCode = String((rpc && rpc.error && (rpc.error.code || rpc.error.status)) || '');
      var finalMsg  = String((rpc && rpc.error && rpc.error.message) || '').toLowerCase();

      var conflictOk =
        finalCode === '409' ||
        finalMsg.indexOf('duplicate') !== -1 ||
        finalMsg.indexOf('already') !== -1 ||
        finalMsg.indexOf('exists') !== -1;

      if (rpc && rpc.error && !conflictOk) {
        console.warn('[ensureUserRow] ensure_user non validé:', rpc.error);
        return false;
      }
    }
...
async function bootstrapAuthAndProfile() {
  ...
  bootstrapAuthAndProfilePromise = (async () => {
    try {
      ...
      if (!ensured) throw new Error('ensureUserRow failed');
      return true;
    } catch (e) {
...
async function getUnlockedThemesCloud() {
  ...
  if (!norm.includes('cyber')) norm.unshift('cyber'); // sécurité UI
  return norm;
}
