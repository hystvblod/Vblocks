...
(function (global) {
  ...
  function isVariantTheme(name) {
    const tname = name || (localStorage.getItem('themeVBlocks') || 'cyber');
  ...
  themeName === 'nature'
    ...
    function changeTheme(themeName) {
      ...
      if (link) link.href = getThemeCssHref(themeName);
    ...
    window.addEventListener('vblocks-theme-changed', (e) => {
      const tname = (e?.detail?.theme) || localStorage.getItem('themeVBlocks') || 'cyber';
    ...
    function safeRedraw() {
      try {
        ...
        try {
          ...
          console.error('[VBlocks Duel] redraw recovery failed', e2);
        }
      }
    }
    ...
    function drawMiniPiece(c, piece) {
      ...
      if (!safePiece || !safePiece.shape) {
        console.warn('[VBlocks Duel] drawMiniPiece skipped invalid piece', piece);
        return;
      }
      ...
      try {
        ...
        shape.forEach((row, y) => row.forEach((val, x) => {
          if (!val) return;
          drawBlockCustom(
            c, x, y, safePiece.letter, cellSize, false,
            isVariantTheme(currentTheme) ? safePiece.variants?.[y]?.[x] : 0
          );
        }));
      } finally {
        c.restore();
